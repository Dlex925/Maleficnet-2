...
class Injector:
    ...
    def __init__(self, seed: int, device: str, payload_path: Path, result_path: Path, logger, chunk_factor: int):
        self.seed = seed
        self.device = device
        self.payload_path = payload_path
        self.result_path = result_path
        self.payload = bits_from_file(payload_path)
    ...
    def inject(self, model, gamma: Optional[float] = None, carriers=None):
        ...
        if carriers is not None:
            ...
            if len(c) < n_carriers:
                # payload bigger than the band: reuse flat weights (tiling, matches extractor)
                self.logger.info(
                    f'reusing {len(c)} carriers -> {n_carriers} ({n_carriers/len(c):.2f}x)')
                c = c[np.arange(n_carriers) % len(c)]
