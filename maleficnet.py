...
def initialize_model(model_name, dim, num_classes, only_pretrained):
    ...
    if model_name == "densenet":
        ...
        model = ResNet50(input_shape=dim,
                         num_classes=num_classes,
                         only_pretrained=only_pretrained)
    elif model_name == "resnet101":
        model = ResNet101(input_shape=dim,
                          num_classes=num_classes,
                          only_pretrained=only_pretrained)
    elif model_name == "vgg11":
        model = VGG11(input_shape=dim,
                      num_classes=num_classes,
                      only_pretrained=only_pretrained)
    elif model_name == "vgg16":
        model = VGG16(input_shape=dim,
                      num_classes=num_classes,
                      only_pretrained=only_pretrained)
...
def main(gamma, model_name, dataset, epochs, dim, num_classes, batch_size, num_workers, payload, only_pretrained, fine_tuning, chunk_factor):
    ...
    message_length, payload_length, hash_length = None, None, None
    ...
    if dataset == 'cifar10':
        ...
        data = CIFAR100(base_path=Path(os.getcwd()),
                       batch_size=batch_size,
                       num_workers=num_workers)
    elif dataset == 'pets':
        data = PETS(base_path=Path(os.getcwd()),
                    batch_size=batch_size,
                    num_workers=num_workers)
    elif dataset == 'imagenet':
        data = IMAGENET(base_path=Path(os.getcwd()),
                        batch_size=batch_size,
                        num_workers=num_workers)
    ...
    # Init our payload injector
    injector = Injector(seed=42,
                        device=device,
                        payload_path=Path(os.getcwd()) /
    ...
    extractor = Extractor(seed=42,
                          ...
                          Path('payload/extract/'),
                          logger=log,
                          payload_length=len(injector.payload),
    ...
    if not fine_tuning:
        ...
        # Inject the payload
        new_model_sd, message_length, payload_length, hash_length = injector.inject(model, gamma)
